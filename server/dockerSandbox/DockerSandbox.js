...
DockerSandbox.prototype.prepare = function (success) {
    ...
    const fullpath = `${this.path + this.folder}`;

    exec("mkdir " + fullpath, function (st) {
        fs.chmod(fullpath, 0777, (err) => { if (err) console.log("Error Found:" + err) });
        fs.writeFile(fullpath + fileSep + sandbox.file_name, sandbox.code, function (err) {
            if (err) {
                console.log(err);
            }
            else {
                console.log(sandbox.langName + " file was saved!");
                fs.chmod(fullpath + fileSep + sandbox.file_name, 0777, (err) => { if (err) console.log("Error Found:" + err) });
                // exec("chmod 777 \'" + fullpath + fileSep + sandbox.file_name + "\'")

                fs.writeFile(fullpath + fileSep + "inputFile", sandbox.stdin_data, function (err) {
                    if (err) {
                        console.log(err);
                    }
                    else {
...
DockerSandbox.prototype.execute = function (success) {
    ...
    var st = this.path + 'DockerTimeout.sh ' + this.timeout_value + 's -e \'NODE_PATH=/usr/local/lib/node_modules\' -i -t -v  ' + this.path + this.folder + ':/usercode ' + this.vm_name
        + " bin/bash -c 'cp /home/Payload/* /usercode ; /usercode/script.sh " + this.compiler_name + ' ' + this.file_name + ' ' + this.output_command + ' ' + this.extra_arguments + " ' ";
    console.log(st);
    ...
    var intid = setInterval(function () {
        ...
        fs.readFile(sandbox.path + sandbox.folder + '/completed', 'utf8', function (err, data) {
            ...
            (async () => {
                try {
                    await del(sandbox.folder);
                } catch (err) {
                    console.error(`Error while deleting ${sandbox.folder}.`);
                }
            })();
            clearInterval(intid);
        });
